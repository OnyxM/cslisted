  ...
  <div class="col-md-9 col-sm-7">
    ...
    <div class="content-page">
       <div class="content-form-page">
          <form role="form" class="form-horizontal form-without-legend" method="POST" action="{{ route('product.update',['id'=>$product->id]) }}" enctype="multipart/form-data">
            <div class="form-group">
              <label class="col-lg-2 control-label" for="first-name">Name <span class="require">*</span></label>
              <div class="col-lg-8">
                <input type="text" id="first-name" class="form-control" value="{{ $product->name }}" name="name">
              </div>
            </div>
                ...
                <select class="form-control js-select2" multiple name="categories[]">
                  @foreach ($categories as $category)
                    <option value="{{ $category->id }}" 
                      @if ($product->categories()->get()->pluck('id')->contains($category->id))
                        selected
                      @endif
            ...
            <div class="form-group">
              ...
              <div class="col-lg-8">
                <input type="text" id="regular-price" class="form-control" value="{{ $product->regular_price }}" name="regular-price">
              </div>
            </div>
            <div class="form-group">
              <label class="col-lg-2 control-label" for="first-name">Discount price</label>
              <div class="col-lg-8">
                <input type="text" id="discount-price" class="form-control" name="discount_price"  value="{{ $product->discount_price }}">
              </div>
            </div>
            <div class="form-group">
                ...
                <select class="form-control js-select2" multiple name="cities[]">
                  @foreach ($cities as $city)
                    <option value="{{ $city->id }}"
                      @if ($product->cities()->get()->pluck('id')->contains($city->id))
                       selected
                      @endif
...
<script>
  window.files = {!! $product->getMedia("products")->map(function($item){
